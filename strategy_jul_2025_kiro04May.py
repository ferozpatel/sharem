...
def checkCriteriaAndTakeTrade():
    ...
    # Premium change per candle (for buying strategy analysis)
...
def takeEntry(isBullish, isBearish, qty, fyers, papertrading):
    ...
    if isBullish:
        ...
        # Option 4: Find hedge at 500-pt interval where premium ≤ 60% of ATM premium
        max_premium = entryPrice * 0.60
        hedge_strike = (syntheticATMStrike // 500) * 500  # snap to nearest 500-pt multiple at or below
        otmPE_found = None
        for _ in range(6):  # check up to 3000 pts away (6 × 500)
