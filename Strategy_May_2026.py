...
qty = 150  # 5 lots x 30 = 150 — default/fallback; overridden by risk-based sizing
sl_point = 50
target_point = 60
...
# Capital ceiling: ~₹4,00,000 available. Hedged spread margin ~₹18k/lot (conservative),
# so 4L / 18k ≈ 22 lots. Set 15 to keep a buffer for margin spikes / premium variation.
...
while x == 1:
    ...
    if now >= custom_time1:
        ...
        if dt1.second <= 1 and dt1.minute % timeFrame == 0:
            ...
            # Logic 1: Morning trap (9:15-9:45 only)
            # Logic 2: Trend convergence (anytime) — CHOI + Total OI both same direction by 10%
            # Logic 3: CHOI trap (anytime) — CHOI dominance + 25% diff
            ...
            # Logic 3: CHOI trap — CEchoi > PEchoi for bull, reverse for bear, + 25% diff
            logic3_bull = _bull_mandatory and (suppResCeChOi > suppResPeChOi) and _choi_pct >= 25
            logic3_bear = _bear_mandatory and (suppResCeChOi < suppResPeChOi) and _choi_pct >= 25
            ...
            if bull_direction_ok and slCount != 2 and dt1.hour <= 15 and SUPP_RES != "NOTRADEZONE" and st == 0 and choi_filter_bull and FUT_LTP > SUPP_RES and IS_CONSECUTIVELY_2TIMES_PCR_INCREASED2:
                ...
                if _entry_mode == "LOGIC3_CHOI_25PCT":
                    print("=" * 60)
                ...
                # Size lots so SL-hit loss ~= FIXED_RISK_PER_TRADE. Uses ATM PE candle median x3.5
                ...
                _sl_pre = round(_opt_range_pre * 3.5) if (_opt_range_pre and _opt_range_pre > 0) else iv_params.get("sl_point", sl_point)
            ...
            # === BEAR ENTRY ===
                ...
                # === RISK-BASED LOT SIZING (pre-entry) ===
                # Size lots so SL-hit loss ~= FIXED_RISK_PER_TRADE. Uses ATM CE candle median x3.5
                # (same SL basis used post-entry). SL/Target logic itself is untouched.
